...
static void _leis_send_set_cmd(smart_air_ctrl_t* ac, u_int8_t cmd, u_int8_t *data, u_int16_t len)
{
	u_int8_t buf[512] = {0};
	
	ucp_leis_pkt_hdr_t *hd = (ucp_leis_pkt_hdr_t *)buf;
	hd->syn1 = 0xff;
	hd->syn2 = 0xaa;
	hd->cmd = cmd;
...
static bool leis_do_lamp_ctrl(smart_air_ctrl_t* ac, cl_rf_lamp_stat_t *src)
{
	u_int8_t buf[512] = {0};
	leis_uart_lamp_ctrl_param_t *dest;
	int len;
	cl_leis_info_t *pv = (cl_leis_info_t *)ac->com_udp_dev_info.device_info;
	cl_rf_lamp_t *rl = &(pv->lamp_stat);
	
    dest = (leis_uart_lamp_ctrl_param_t*)buf;
    len = sizeof(*dest);
   ...
   // dest->type = 0x0;
	...
	_leis_send_set_cmd(ac, 2, buf, sizeof(*dest));

	return true;
}

bool leis_scm_proc_notify(user_t* user,cl_notify_pkt_t *pkt, RS *ret)
{
	bool res = true;
	...
	switch (info->action){
		case ACT_LEIS_CTRL_LAMP:
			leis_do_lamp_ctrl(ac, (cl_rf_lamp_stat_t *)cci_pointer_data(info));
			break;
...
static bool _leis_do_update(smart_air_ctrl_t* ac,u_int8_t* pcmd, u_int16_t cmd_len)
{
	ucp_leis_pkt_hdr_t* hdr = (ucp_leis_pkt_hdr_t*)pcmd;
	leis_uart_lamp_ctrl_param_t* pkt = (leis_uart_lamp_ctrl_param_t*)&hdr[1];
	cl_leis_info_t *pv = (cl_leis_info_t *)ac->com_udp_dev_info.device_info;
	cl_rf_lamp_t *rl = &(pv->lamp_stat);
	u_int8_t m,n;
	u_int16_t org_rgb_l, tmp;

	log_debug("leis get cmd_len %u\n", cmd_len);
...
#if 0
{
	...
	leis_do_lamp_ctrl(ac, &sat);
}
#endif


	if (hdr->cmd != 3) {
		log_err(false, "not leis push stat\n");
		return false;
	}
...
static bool _leis_scm_update_tlv_data(smart_air_ctrl_t* air_ctrl,u_int8_t action,ucp_obj_t* obj)
{
	uc_tlv_t* tlv;
	...
	while (remain >= sizeof(uc_tlv_t) && (u_int32_t)remain >= sizeof(uc_tlv_t) + tlv->len) {
		...
		case TLV_TYPE_SCM_COMMAND:
			_leis_do_update(air_ctrl,(u_int8_t*)(tlv+1),tlv->len);
			break;
		default:
			break;
		}
...
bool leis_scm_update_data(smart_air_ctrl_t* air_ctrl,u_int8_t action,ucp_obj_t* obj)
{
	bool ret = false;
	switch (obj->attr) {
		case UCAT_IA_TT_ALLSTATE:
			ret = _leis_scm_update_tlv_data(air_ctrl,action,obj);
			break;
		case UCAT_IA_TT_CMD:
			break;
...
bool leis_bulid_priv_dev_info(smart_air_ctrl_t* ac, cl_com_udp_device_data* di)
{
	cl_leis_info_t* priv;
	cl_leis_info_t* info;
	...
	priv = (cl_leis_info_t*)ac->com_udp_dev_info.device_info;
...
CLIB_API RS cl_leis_lamp_ctrl(cl_handle_t dev_handle, cl_rf_lamp_stat_t* request)
{
	CL_CHECK_INIT;
	
	return cl_send_var_data_notify(&cl_priv->thread_main, dev_handle, CLNE_COMMON_UDP_LEIS, ACT_LEIS_CTRL_LAMP, (u_int8_t*)request, sizeof(*request));
}
