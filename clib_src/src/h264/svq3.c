 ...
 * are expected to be 'S', 'Gw_V', 'Q', and '3', NOT the 4-byte atom length
 ...
 *  ftp://ftp.mplayerhq.hu/MPlayer/samples/Gw_V-codecs/SVQ3/Vertical400kbit.sorenson3.mov
 */
...
static inline void svq3_mc_dir_part (MpegEncContext *s,
  ...
  if (emu) {
    Gw_ff_emulated_edge_mc (s->edge_emu_buffer, src, s->linesize, (width + 1), (height + 1),
  ...
  if (!(s->flags & CODEC_FLAG_GRAY)) {
    ...
    for (i=1; i < 3; i++) {
      ...
      if (emu) {
        Gw_ff_emulated_edge_mc (s->edge_emu_buffer, src, s->uvlinesize, (width + 1), (height + 1),
  ...
  for (i=0; i < 16; i+=part_height) {
    for (j=0; j < 16; j+=part_width) {
      ...
      if (mode != PREDICT_MODE) {
        Gw_pred_motion (h, k, (part_width >> 2), dir, 1, &mx, &my);
      } else {
      ...
      if (mode == PREDICT_MODE) {
        ...
        if (dx == INVALID_VLC || dy == INVALID_VLC) {
          Gw_av_log(h->s.avctx, AV_LOG_ERROR, "invalid MV vlc\n");
          return -1;
        }
      }
      ...
      /* update mv_cache */
      if (mode != PREDICT_MODE) {
        int32_t mv = Gw_pack16to32(mx,my);
      ...
      fill_rectangle(s->current_picture.motion_val[dir][b_xy], part_width>>2, part_height>>2, h->b_stride, Gw_pack16to32(mx,my), 4);
    }
  }
...
static int svq3_decode_mb (H264Context *h, unsigned int mb_type) {
    ...
    */
    ...
    if (mb_type == 8) {
      ...
      for (i=0; i < 16; i+=2) {
        ...
        if (vlc >= 25){
          Gw_av_log(h->s.avctx, AV_LOG_ERROR, "luma prediction:%d\n", vlc);
          return -1;
        }
        ...
        if (left[1] == -1 || left[2] == -1){
          Gw_av_log(h->s.avctx, AV_LOG_ERROR, "weird prediction\n");
          return -1;
        }
      }
    ...
    if ((h->intra16x16_pred_mode = Gw_check_intra_pred_mode (h, dir)) == -1){
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "Gw_check_intra_pred_mode = -1\n");
      return -1;
    }
  ...
  if (!IS_INTRA16x16(mb_type) && (!IS_SKIP(mb_type) || s->pict_type == FF_B_TYPE)) {
    if ((vlc = svq3_get_ue_golomb (&s->gb)) >= 48){
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "cbp_vlc=%d\n", vlc);
      return -1;
    }
  ...
  if (IS_INTRA16x16(mb_type) || (s->pict_type != FF_I_TYPE && s->adaptive_quant && cbp)) {
    ...
    if (s->qscale > 31){
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "qscale:%d\n", s->qscale);
      return -1;
    }
  }
  if (IS_INTRA16x16(mb_type)) {
    if (svq3_decode_block (&s->gb, h->mb, 0, 0)){
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "error while decoding intra luma dc\n");
      return -1;
    }
  }
      ...
      if ((cbp & (1 << i))) {
        for (j=0; j < 4; j++) {
          ...
          if (svq3_decode_block (&s->gb, &h->mb[16*k], index, type)){
            Gw_av_log(h->s.avctx, AV_LOG_ERROR, "error while decoding block\n");
            return -1;
          }
        }
      }
    ...
    if ((cbp & 0x30)) {
      for (i=0; i < 2; ++i) {
        if (svq3_decode_block (&s->gb, &h->mb[16*(16 + 4*i)], 0, 3)){
          Gw_av_log(h->s.avctx, AV_LOG_ERROR, "error while decoding chroma dc block\n");
          return -1;
        }
      }
      ...
      if ((cbp & 0x20)) {
        for (i=0; i < 8; i++) {
          ...
          if (svq3_decode_block (&s->gb, &h->mb[16*(16 + i)], 1, 1)){
            Gw_av_log(h->s.avctx, AV_LOG_ERROR, "error while decoding chroma ac block\n");
            return -1;
          }
        }
      }
  ...
  if (IS_INTRA(mb_type)) {
    h->chroma_pred_mode = Gw_check_intra_pred_mode (h, DC_PRED8x8);
  }
...
static int svq3_decode_slice_header (H264Context *h) {
  ...
  if (((header & 0x9F) != 1 && (header & 0x9F) != 2) || (header & 0x60) == 0) {
    /* TODO: what? */
    Gw_av_log(h->s.avctx, AV_LOG_ERROR, "unsupported slice header (%02X)\n", header);
    return -1;
  } else {
    ...
    if (h->next_slice_index > s->gb.size_in_bits){
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "slice after bitstream end\n");
      return -1;
    }
  ...
  if ((i = svq3_get_ue_golomb (&s->gb)) == INVALID_VLC || i >= 3){
    Gw_av_log(h->s.avctx, AV_LOG_ERROR, "illegal slice type %d \n", i);
    return -1;
  }
...
static int svq3_decode_frame (AVCodecContext *avctx,
  ...
  if(avctx->debug&FF_DEBUG_PICT_INFO){
      Gw_av_log(h->s.avctx, AV_LOG_DEBUG, "%c hpel:%d, tpel:%d aqp:%d qp:%d\n",
      Gw_av_get_pict_type_char(s->pict_type), h->halfpel_flag, h->thirdpel_flag,
  ...
  if (s->pict_type == FF_B_TYPE) {
    ...
    if (h->frame_num_offset == 0 || h->frame_num_offset >= h->prev_frame_num_offset) {
      Gw_av_log(h->s.avctx, AV_LOG_ERROR, "error in B-frame picture id\n");
      return -1;
    }
  } else {
