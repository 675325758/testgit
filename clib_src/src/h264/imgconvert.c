...
#define FF_COLOR_YUV      2 /**< YUV color space. 16 <= Y <= 235, 16 <= Gw_U, Gw_V <= 240 */
#define FF_COLOR_YUV_JPEG 3 /**< YUV color space. 0 <= Y <= 255, 0 <= Gw_U, Gw_V <= 255 */
...
int Gw_ff_fill_linesize(AVPicture *picture, int pix_fmt, int width)
{
    int w2;
...
int Gw_ff_fill_pointer(AVPicture *picture, uint8_t *ptr, int pix_fmt,
                    int height)
{
    int size, h2, size2;
    const PixFmtInfo *pinfo;
                   ...
                   int pix_fmt, int width, int height)
{
    ...
    if(Gw_avcodec_check_dimensions(NULL, width, height))
        return -1;

    if (Gw_ff_fill_linesize(picture, pix_fmt, width))
        return -1;

    return Gw_ff_fill_pointer(picture, ptr, pix_fmt, height);
}
                     ...
                     unsigned char *dest, int dest_size)
{
    ...
    int size = Gw_avpicture_get_size(pix_fmt, width, height);
...
int Gw_avpicture_get_size(int pix_fmt, int width, int height)
{
    ...
    return Gw_avpicture_fill(&dummy_pict, NULL, pix_fmt, width, height);
}
                                      ...
                                      int loss_mask)
{
    ...
    for(i = 0;i < PIX_FMT_NB; i++) {
        if (pix_fmt_mask & (1 << i)) {
            loss = Gw_avcodec_get_pix_fmt_loss(i, src_pix_fmt, has_alpha) & loss_mask;
            if (loss == 0) {
                              ...
                              int has_alpha, int *loss_ptr)
{
 ...
 found:
    if (loss_ptr)
        *loss_ptr = Gw_avcodec_get_pix_fmt_loss(dst_pix_fmt, src_pix_fmt, has_alpha);
    return dst_pix_fmt;
}

void Gw_ff_img_copy_plane(uint8_t *dst, int dst_wrap,
...
int Gw_ff_get_plane_bytewidth(enum PixelFormat pix_fmt, int width, int plane)
{
    int bits;
    const PixFmtInfo *pf = &pix_fmt_info[pix_fmt];
              ...
              int pix_fmt, int width, int height)
{
    ...
    case FF_PIXEL_PLANAR:
        for(i = 0; i < pf->nb_channels; i++) {
            int w, h;
            int bwidth = Gw_ff_get_plane_bytewidth(pix_fmt, width, i);
            w = width;
            h = height;
            if (i == 1 || i == 2) {
                w >>= pf->x_chroma_shift;
            ...
            Gw_ff_img_copy_plane(dst->data[i], dst->linesize[i],
                           src->data[i], src->linesize[i],
                           bwidth, h);
        }
        break;
    case FF_PIXEL_PALETTE:
        Gw_ff_img_copy_plane(dst->data[0], dst->linesize[0],
                       src->data[0], src->linesize[0],
                       width, height);
        /* copy the palette */
        Gw_ff_img_copy_plane(dst->data[1], dst->linesize[1],
...
/* 2x2 -> 1x1 */
void Gw_ff_shrink22(uint8_t *dst, int dst_wrap,
                     const uint8_t *src, int src_wrap,
                     int width, int height)
{
    int w;
...
/* 4x4 -> 1x1 */
void Gw_ff_shrink44(uint8_t *dst, int dst_wrap,
                     const uint8_t *src, int src_wrap,
                     int width, int height)
{
    int w;
...
/* 8x8 -> 1x1 */
void Gw_ff_shrink88(uint8_t *dst, int dst_wrap,
                     const uint8_t *src, int src_wrap,
                     int width, int height)
{
    int w, i;
...
int Gw_avpicture_alloc(AVPicture *picture,
                           int pix_fmt, int width, int height)
{
    int size;
    void *ptr;

    size = Gw_avpicture_get_size(pix_fmt, width, height);
    if(size<0)
        goto fail;
    ptr = Gw_av_malloc(size);
    if (!ptr)
        goto fail;
    Gw_avpicture_fill(picture, ptr, pix_fmt, width, height);
    return 0;
 fail:
...
int Gw_av_picture_pad(AVPicture *dst, const AVPicture *src, int height, int width,
...
void Gw_img_copy(AVPicture *dst, const AVPicture *src,
              int pix_fmt, int width, int height)
{
    Gw_av_picture_copy(dst, src, pix_fmt, width, height);
}

int Gw_img_crop(AVPicture *dst, const AVPicture *src,
              int pix_fmt, int top_band, int left_band)
{
    return Gw_av_picture_crop(dst, src, pix_fmt, top_band, left_band);
}

int Gw_img_pad(AVPicture *dst, const AVPicture *src, int height, int width,
            int pix_fmt, int padtop, int padbottom, int padleft, int padright,
            int *color)
{
    return Gw_av_picture_pad(dst, src, height, width, pix_fmt, padtop, padbottom, padleft, padright, color);
}
#endif
...
{
    int i;
    uint8_t *cm = Gw_ff_cropTbl + MAX_NEG_CROP;
...
int Gw_img_convert(AVPicture *dst, int dst_pix_fmt,
                ...
                int src_width, int src_height)
{
    ...
    if (src_pix_fmt == dst_pix_fmt) {
        ...
        Gw_av_picture_copy(dst, src, dst_pix_fmt, dst_width, dst_height);
        return 0;
    }
    ...
    if (is_yuv_planar(dst_pix) && is_yuv_planar(src_pix)) {
        ...
        switch(xy_shift) {
        case 0x00:
            resize_func = Gw_ff_img_copy_plane;
            break;
        case 0x10:
        ...
        case 0x01:
            ...
            break;
        case 0x11:
            resize_func = Gw_ff_shrink22;
            break;
        case 0x22:
            resize_func = Gw_ff_shrink44;
            break;
        case 0xf0:
            resize_func = grow21;
            break;
 ...
 no_chroma_filter:
    ...
    if (src_pix_fmt == int_pix_fmt)
        return -1;
    if (Gw_avpicture_alloc(tmp, int_pix_fmt, dst_width, dst_height) < 0)
        return -1;
    ret = -1;
    if (Gw_img_convert(tmp, int_pix_fmt,
                    src, src_pix_fmt, src_width, src_height) < 0)
        goto fail1;
    if (Gw_img_convert(dst, dst_pix_fmt,
                    tmp, int_pix_fmt, dst_width, dst_height) < 0)
        goto fail1;
    ret = 0;
 fail1:
    Gw_avpicture_free(tmp);
    return ret;
}
#endif
                                             ...
                                             int width, int height)
{
    ...
    int y;
    uint8_t *buf;
    buf = (uint8_t*)Gw_av_malloc(width);
