...
extern const uint8_t Gw_ff_golomb_vlc_len[512];
extern const uint8_t Gw_ff_ue_golomb_vlc_code[512];
extern const  int8_t Gw_ff_se_golomb_vlc_code[512];
extern const uint8_t Gw_ff_ue_golomb_len[256];

extern const uint8_t Gw_ff_interleaved_golomb_vlc_len[256];
extern const uint8_t Gw_ff_interleaved_ue_golomb_vlc_code[256];
extern const  int8_t Gw_ff_interleaved_se_golomb_vlc_code[256];
extern const uint8_t Gw_ff_interleaved_dirac_golomb_vlc_code[256];
...
static inline int get_ue_golomb(GetBitContext *gb){
    ...
    if(buf >= (1<<27)){
        buf >>= 32 - 9;
        LAST_SKIP_BITS(re, gb, Gw_ff_golomb_vlc_len[buf]);
        CLOSE_READER(re, gb);

        return Gw_ff_ue_golomb_vlc_code[buf];
    }else{
...
static inline int svq3_get_ue_golomb(GetBitContext *gb){
    ...
    if(buf&0xAA800000){
        buf >>= 32 - 8;
        LAST_SKIP_BITS(re, gb, Gw_ff_interleaved_golomb_vlc_len[buf]);
        CLOSE_READER(re, gb);

        return Gw_ff_interleaved_ue_golomb_vlc_code[buf];
    }else{
        int ret = 1;

        while (1) {
            buf >>= 32 - 8;
            LAST_SKIP_BITS(re, gb, FFMIN(Gw_ff_interleaved_golomb_vlc_len[buf], 8));

            if (Gw_ff_interleaved_golomb_vlc_len[buf] != 9){
                ret <<= (Gw_ff_interleaved_golomb_vlc_len[buf] - 1) >> 1;
                ret |= Gw_ff_interleaved_dirac_golomb_vlc_code[buf];
                break;
            }
            ret = (ret << 4) | Gw_ff_interleaved_dirac_golomb_vlc_code[buf];
            UPDATE_CACHE(re, gb);
            buf = GET_CACHE(re, gb);
        }
...
static inline int get_se_golomb(GetBitContext *gb){
    ...
    if(buf >= (1<<27)){
        ...
        return Gw_ff_se_golomb_vlc_code[buf];
    }else{
        log= 2*av_log2(buf) - 31;
        buf>>= log;
...
static inline int svq3_get_se_golomb(GetBitContext *gb){
    ...
    if(buf&0xAA800000){
        ...
        return Gw_ff_interleaved_se_golomb_vlc_code[buf];
    }else{
...
static inline int get_ue(GetBitContext *s, char *file, const char *func, int line){
    ...
    Gw_av_log(NULL, AV_LOG_DEBUG, "%5d %2d %3d ue  @%5d in %s %s:%d\n", bits, len, i, pos, file, func, line);
...
static inline int get_se(GetBitContext *s, char *file, const char *func, int line){
    ...
    Gw_av_log(NULL, AV_LOG_DEBUG, "%5d %2d %3d se  @%5d in %s %s:%d\n", bits, len, i, pos, file, func, line);
...
static inline int get_te(GetBitContext *s, int r, char *file, const char *func, int line){
    ...
    Gw_av_log(NULL, AV_LOG_DEBUG, "%5d %2d %3d te  @%5d in %s %s:%d\n", bits, len, i, pos, file, func, line);
...
#endif
    if(i<256)
        put_bits(pb, Gw_ff_ue_golomb_len[i], i+1);
    else{
        e= av_log2(i+1);
...
//    if (i>32767 || i<-32767)
//        Gw_av_log(NULL,AV_LOG_ERROR,"value out of range %d\n", i);
#if 0
    if(i<=0) i= -2*i;
    else     i=  2*i-1;
#elif 1
