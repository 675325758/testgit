...
void Gw_ff_er_add_slice(MpegEncContext *s, int startx, int starty, int endx, int endy, int status){
    ...
    if(start_i > end_i || start_xy > end_xy){
        Gw_av_log(s->avctx, AV_LOG_ERROR, "internal error, slice end before start\n");
        return;
    }
...
void Gw_ff_er_frame_end(MpegEncContext *s){
    ...
    if(s->current_picture.motion_val[0] == NULL){
        Gw_av_log(s->avctx, AV_LOG_ERROR, "Warning MVs not available\n");

        for(i=0; i<2; i++){
            pic->ref_index[i]= Gw_av_mallocz(size * sizeof(uint8_t));
            pic->motion_val_base[i]= Gw_av_mallocz((size+4) * 2 * sizeof(uint16_t));
    ...
    if(s->avctx->debug&FF_DEBUG_ER){
        for(mb_y=0; mb_y<s->mb_height; mb_y++){
            for(mb_x=0; mb_x<s->mb_width; mb_x++){
                ...
                Gw_av_log(s->avctx, AV_LOG_DEBUG, "%2X ", status);
            }
    ...
    Gw_av_log(s->avctx, AV_LOG_INFO, "concealing %d DC, %d AC, %d MV errors\n", dc_error, ac_error, mv_error);
