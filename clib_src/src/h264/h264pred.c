...
static inline void pred16x16_plane_compat_c(uint8_t *src, int stride, const int svq3, const int rv40){
  ...
  int H = src0[1] - src0[-1];
  int Gw_V = src1[0] - src2[ 0];
  for(k=2; k<=8; ++k) {
    src1 += stride; src2 -= stride;
    H += k*(src0[k] - src0[-k]);
    Gw_V += k*(src1[0] - src2[ 0]);
  }
  if(svq3){
    H = ( 5*(H/4) ) / 16;
    Gw_V = ( 5*(Gw_V/4) ) / 16;
  ...
  a = 16*(src1[0] + src2[16] + 1) - 7*(Gw_V+H);
  for(j=16; j>0; --j) {
    int b = a;
    a += Gw_V;
    for(i=-16; i<0; i+=4) {
 ...
 */
void Gw_ff_h264_pred_init(H264PredContext *h, int codec_id){
